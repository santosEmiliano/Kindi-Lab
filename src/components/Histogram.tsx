...
export function Histogram({ observed, expected, labels }: HistogramProps) {
  ...
  const maxValue = Math.max(0.02, ...observed, ...expected) * 1.12
  ...
  return (
    <>
      ...
      <details className="hist-table">
        ...
        <table>
          <caption>
            Frecuencia por letra: texto cifrado frente al español esperado
          </caption>
          <thead>
            <tr>
          ...
          <tbody>
            {labels.map((label, index) => (
              <tr key={label}>
                ...
                <td>{pct(observed[index])}</td>
                <td>{pct(expected[index])}</td>
              </tr>
            ))}
          </tbody>
        </table>
